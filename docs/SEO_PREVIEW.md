...
- `index.html` é o modelo de origem; `npm run build` cria a entrega em `dist/`. A Vercel usa esse diretório e mantém as funções de `/api`.
...
- Sitemap com os 41 destinos de produção; robots permite rastreamento. A proteção `noindex` do preview deve ser confirmada no cabeçalho da implantação Vercel.
...
- Estoque agregado entre fragrâncias, escape de texto, chaves internas seguras e cotação invalidada ao mudar itens.
...
`npm test`: 42 testes aprovados, incluindo 14 testes de checkout preexistentes, políticas, metadados e estado do carrinho.
...
O navegador remoto não acessa o servidor local (ERR_BLOCKED_BY_CLIENT); a verificação visual será realizada na implantação de preview. O painel `/__preview-qa/` só é gerado em ambiente Preview e permite conferir o mesmo site em quadros de 360, 390, 768 e 1366 px. Não entra no sitemap nem no build de produção.

## Pendências para concluir a revisão

- Endereço da implantação, confirmação de build/APIs, `noindex`, status HTTP e revisão visual.
...
2. Revisar branch/PR e implantação gerada pela integração GitHub–Vercel, sem merge em main.
